...
if (strlen($search_string) >= 1 && $search_string !== ' ') {
	...
	if($_POST['brand_url'] !="")
	{
		$query = 'SELECT distinct category_path,display_term,url FROM search WHERE search_term LIKE "%'.$search_string.'%" and manufacturer_name LIKE "%'.$url_string.'%"  Limit 20';
	}
	else
	{
		if($url_string == "golf-courses")
		...
		else if($url_string == "")
		{
			...
			$query = 'SELECT distinct category_path,display_term,url FROM search WHERE search_term LIKE "%'.$search_string.'%" Limit 10';
			//$url_string = "golf-clubs";
		}
		else
		{
			$query = 'SELECT distinct category_path,display_term,url FROM search WHERE search_term LIKE "%'.$search_string.'%" and (category_path LIKE "%'.$url_string.'%" or url LIKE "%brand%" ) Limit 10';		
		}
	}
...
/* for filtering in homepage */
				...
				if($url_string == '')
				{
					...
					{				
						...
						$html .= '<a href="urlString">';
						$html .= '<h3 style="font-size:13px">functionString</h3>';
						$html .= '<h3 style="font-size:10px ;   margin-top: 7px;    margin-left: 5px;">nameString</h3>';
						$html .= '<h4 style="float:right">typeString</h4>';
						$html .= '</a>';
						$html .= '</li>';
					}
