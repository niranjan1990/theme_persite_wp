...
echo "<!-- Template Name: Sub Category Landing Page others ,page_sub_category_subpage_others.php -->";
...
<?php //echo "Category /category/category/category.html"; ?>
...
$seo_title = str_replace("[category_name]",str_replace('-',' ',$parts2[0]),$CATEGORYTITLETHIRDLEVEL);
$seo_description = str_replace("[category_name]",str_replace('-',' ',$parts2[0]),$CATEGORYDESCRIPTIONTHIRDLEVEL);
$seo_keywords = str_replace("[category_name]",str_replace('-',' ',$parts2[0]),$CATEGORYKEYWORDSTHIRDLEVEL);
}else{
...
if($DEEPCATEGORIES == "1")
{
	...
	{ 
		site_review_category_list_outdoor_sub($_SESSION['archive'],3); 
	} 
}
else
{
...
?>
			...
			{ /* NODEID is also database driven */
				...
				{	
					...
					{ 
							...
							{ 
								if(strpos($Categories[$j]->nodeid, $Categories[$i]->nodeid ) !== false && strpos($Categories[$j]->nodeid, $NODEID ) == 0 && $Categories[$j]->node_level >= 5 ) 
								{
	...
	<div class="golf-club-list"> Select the brand alphabetically 
		...
		if($DEEPCATEGORIES == '1')
		{
			if(function_exists('site_review_brand_list_outdoor_brand_three')){ 
				site_review_brand_list_outdoor_brand_three(1, $Category->url_safe_category_name, $BrandIdList); /*** print out brand list alphabetically */ 
			} 
		}
		else
		{
