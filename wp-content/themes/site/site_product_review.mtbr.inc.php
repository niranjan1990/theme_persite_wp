			...
			else if($_POST['submit']=="Done")
			{
				...
				else
				{
					...
					if(isset($_COOKIE['bb_userid']))
					{
						...
						$sqlinsert = mysql_query("INSERT INTO reviews (reviewerip, ProductID, date_created, user_screen_name, model,channelid,valid,overall_rating,pros,cons,year_purchased,price,product_type,vbulletinid)
		VALUES ('".$_SERVER['REMOTE_ADDR']."','".$_SESSION['ProductID']."','".date('Y-m-d H:i:s')."','".$_COOKIE['bb_username']."','".$_SESSION['pname']."','".$_SESSION['channelid']."','1','".$_SESSION['rating']."','".$_SESSION['pros']."','".$_SESSION['cons']."','".$_SESSION['year']."','".$_SESSION['price']."','".$_SESSION['purchase']."','".$_COOKIE['bb_userid']."')");
...
<?php
			...
			{
				echo "<label>Tell us what you liked about this golf course.</label>";
			}
			else
			{
...
<?php } ?>
			...
			?>" name="pros"  pattern=".+?(?:[\s'].+?){2,}" ><?php if($_GET['reviewid']){ echo $res1['pros'];}else{} ?></textarea>
			</div>
			<div id="dislikefeed">
<?php if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 8.') != false || strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 9.') != false) { ?>
<?php
			if($channel_type[1]=="golf-courses")
			{
				echo "<label>Tell us what you disliked about this golf course.</label>";
			}
			else
			{
...
<?php } ?>
			...
			?>" name="cons" pattern=".+?(?:[\s'].+?){2,}" ><?php if($_GET['reviewid']){ echo $res1['cons'];}else{} ?></textarea>
			</div>
...
<?php if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 8.') != false || strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 9.') != false) { ?>
<div id="price" style="margin-top: 28px;foat:left;">
<label>Price Paid?</label></br>
<?php }else{ ?>
<div id="price" style="margin-top: 6px;foat:left;">
<?php } ?>
