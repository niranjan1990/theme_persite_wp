...
echo "<!-- Template Name: Sub Category Landing Page,page-sub-category-subpage.php -->";
...
<?php $parts3 = explode( '.html', $parts1[2] ); ?>
...
$seo_title_temp = str_replace("[category_name]",str_replace('-',' ',$parts2[0]),$CATEGORYSUBCATETGORYTITLE);
$seo_description_temp = str_replace("[category_name]",str_replace('-',' ',$parts2[0]),$CATEGORYSUBCATEGORYDESCRIPTION);
$seo_keywords_temp = str_replace("[category_name]",str_replace('-',' ',$parts2[0]),$CATEGORYSUBCATEGORYKEYWORDS);
$seo_title = str_replace("[subcategory_name]",str_replace('-',' ',$parts3[0]),$seo_title_temp);
$seo_description = str_replace("[subcategory_name]",str_replace('-',' ',$parts3[0]),$seo_description_temp);
$seo_keywords = str_replace("[subcategory_name]",str_replace('-',' ',$parts3[0]),$seo_keywords_temp);
...
if($DEEPCATEGORIES=="1")
{
	...
	{
		site_review_product_list_outdoor_new();
	}
}
else
{
...
if($DEEPCATEGORIES=="1")
{
	...
	{ 
		$lvl = 2;
		site_review_category_list_outdoor($_SESSION['archive'],$lvl); 
	}
}
else
{
...
?>
	...
	<div id="content-left" class="col-sm-8">
		<div class="inner">			
			...
			<h1><?php echo str_replace('-',' ', $parts2[0]); ?> - <?php echo str_replace('-',' ', $parts3[0]); ?></h1>
			<div class="golf-clubs">
<div class="parent" style="float: left;margin: 25px 0px;width:100%;">
			...
			{ 		
				...
				{	
					...
					if(strpos($Categories[$i]->nodeid, $NODEID ) !== false && strpos($Categories[$i]->nodeid, $NODEID ) == 0 && $Categories[$i]->node_level == 4 ) 
					{ 
							for($j=0;$j<count($Categories);$j++) 
							{ 
								if(strpos($Categories[$j]->nodeid, $Categories[$i]->nodeid ) !== false && strpos($Categories[$j]->nodeid, $NODEID ) == 0 && $Categories[$j]->node_level >= 4 ) 
								{
						...
						<div style="float:left;margin:5px 5px 5px 0px;border-radius: 5px;padding:4px 4px 4px 0px;"><a class="BtnWriteReview" id="featured-nav" style="font-weight: normal;text-decoration:none;padding: 4px;/*background-color: rgb(9, 111, 0);*/border-radius: 2px;color: #fff;" href="<?php echo $parts3[0].'/'.$Categories[$i]->url_safe_category_name.".html" ?>"><?php if(function_exists('cr_category_name')){ cr_category_name($Categories[$i]->category_name); ?> (<?php echo number_format($catcount); } ?>)</a></div>
						<?php } ?>
				...
				{
					if(strpos($Categories[$i]->nodeid, $NODEID ) !== false && strpos($Categories[$i]->nodeid, $NODEID ) == 0 && $Categories[$i]->node_level == 4 && number_format( $Categories[$i]->product_count) != 0) 
					{ 
						$cp = explode("/",$Categories[$i]->category_path);
						$cp1 = explode("/",strrev($Categories[$i]->category_path));
						if($cp[1] == $parts3[0])
						{
						?>
						<div style="float:left;margin:5px 5px 5px 0px;border-radius: 5px;padding:4px 4px 4px 0px;">
							<a class="BtnWriteReview" id="featured-nav" style="font-weight: normal;text-decoration:none;padding: 4px;/*background-color: rgb(9, 111, 0);*/border-radius: 2px;color: #fff;" href="<?php echo $parts3[0] ."/"; echo strrev($cp1[0]); ?>.html"><?php if(function_exists('cr_category_name')){ cr_category_name($Categories[$i]->category_name); ?> (<?php echo number_format($Categories[$i]->product_count); } ?>)</a>
						</div>
						<?Php 
							$CatName = strtolower( $Categories[$i]->category_name );
							$CatName = str_replace('&', 'and', $CatName ); 
							$BrandList[] = str_replace(' ', '-', $CatName ); 
							$BrandIdList[] = $Categories[$i]->categoryid; 
	...
	<div><p class="subheadertext" style="margin-top: 0px;">Select the brand you are interested in</p>			
	<div class="golf-club-list"> Select the brand alphabetically 	
		...
		if($DEEPCATEGORIES=="1")
		{
			...
			{ 
				site_review_brand_list_outdoor_subcat(1, $Category->url_safe_category_name, $BrandIdList); 
...
Sort by
			...
			<div style="float:right;"><?php  $Category->url_safe_category_name = $parts3[0]; $GRclass->cr_category_pagination_components($Category,$_GET['pg']); ?></div>
		...
		<div class="product-list-nav clearfix"><div style="float:right;"><?php  $Category->url_safe_category_name = $parts3[0]; $GRclass->cr_category_pagination_components($Category,$_GET['pg']); ?></div>
				<div style="float:right;">Showing <?php $GRclass->cr_category_showing($Category, $_GET['pg']); ?> of <?php echo $Category->product_count; ?> <!--<?php echo $Category->category_name;  ?>-->&nbsp;</div>
			</div>
			<?php
			}
			...
			{
			?>
		<div class="product-list-nav clearfix"><div style="float:right;"><?php  $Category->url_safe_category_name = $parts3[0]; $GRclass->cr_category_pagination_components($Category1,$_GET['pg']); ?></div>
